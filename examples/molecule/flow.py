    ...
    def process(self):
        ...
        if self.fmt=='npy_single':
            for i in dataset[0]:
                ...
                dataset[0].to_deepmd_npy(self.prefix + "/train"+f"/{name}")
                
            for i in dataset[1]:
                name=extract_nonzero_elements(i.formula)
                dataset[1].to_deepmd_npy(self.prefix + "/val"+f"/{name}")
        if self.fmt=='npy_mix':
            dataset[0].to_deepmd_npy_mixed(self.prefix + "/train")
            dataset[1].to_deepmd_npy_mixed(self.prefix + "/val")
        elif self.fmt=='lmdb':
            # train=dataset[0].to_ase_structure(self.prefix + "/train")
            # val=dataset[1].to_ase_structure(self.prefix + "/val")
            ...
            def tolmdb(file,data):
                ...
                for i in data:
                    name=extract_nonzero_elements(i.formula)
                    data=i.to_ase_structure()
                    data_objects = a2g.convert_all(data, disable_tqdm=True)
                    db = lmdb.open(
...
def parse_arguments():
    ...
    parser.add_argument("operation", choices=["opt", "md", "convcheck", "mdcheck", "dpdata",'plot'], help="Specify the operation to perform.")

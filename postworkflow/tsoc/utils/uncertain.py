...
def calculate_pointwise_errors(checkpoint_energies, vasp_energy):
    mse_list = []
    rmse_list = []

    for i in range(len(vasp_energy)):
        errors = [(energies[i] - vasp_energy[i]) ** 2 for energies in checkpoint_energies]
        mse = np.mean(errors)
        rmse = np.sqrt(mse)
        mse_list.append(mse)
...
def main():
    ...
    # Read VASP results
    structure_paths, vasp_energy, vasp_forces = read_vasp_energy_forces(args.vasp_results_path, args.n_images)
    vasp_raw_energy = vasp_energy
    # Normalize VASP energies to the first frame
    vasp_energy = [e - vasp_energy[0] for e in vasp_energy]
    ...
    for checkpoint_path in args.checkpoint_paths:
        ...
        # Normalize energies to the first frame
